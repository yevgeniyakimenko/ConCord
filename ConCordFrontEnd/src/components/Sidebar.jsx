    ...
    <div className="sidebar text-left sm:w-1/6 pb-4 sm:pt-4 flex flex-col">
      ...
      {selectedChannel && (
        <select
          ...
          onChange={(e) => {
            const newSelChannel = channels.find(
              (channel) => channel.id == e.target.value,
            )
            onSelectChannel(newSelChannel)
          }}
          className="sm:hidden border bg-white dark:bg-black rounded-md border-slate-500 shadow-md shadow-slate-400 dark:shadow-none px-4 py-3 mb-4"
        >
          {channels &&
      ...
      <form>
        <div className="flex sm:flex-col sm:items:">
          ...
          <button
            type="button"
            className="sm:w-16 border rounded-md bg-white dark:bg-black hover:bg-orange-200 hover:dark:bg-orange-900 border-slate-500 shadow-md shadow-slate-400 dark:shadow-none px-3 py-2"
            onClick={async () => {
